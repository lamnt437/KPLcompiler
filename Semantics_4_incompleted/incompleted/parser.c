...
Type* compileLValue(void) { // lvalue - memory location that identifying an object
...
void compileAssignSt(void) {
  ...
  eat(SB_ASSIGN);
  Type *rvalueType = compileExpression();
  checkBasicType(lvalueType);
  checkBasicType(rvalueType);
  checkTypeEquality(lvalueType, rvalueType);
}
...
void compileForSt(void) {
  ...
  Object *forVar = checkDeclaredVariable(currentToken->string);
  Type *forVarType = forVar->varAttrs->type;
  checkBasicType(forVarType);

  eat(SB_ASSIGN);
  Type *forVarRvalueType = compileExpression();
  checkBasicType(forVarRvalueType);
  checkTypeEquality(forVarType, forVarRvalueType);

  eat(KW_TO);
  Type *toVarRvalueType = compileExpression();
  checkBasicType(toVarRvalueType);
  checkTypeEquality(forVarType, toVarRvalueType);
...
void compileArgument(Object* param) {
  ...
  // ideas: if reference eat token, compile Lvalue, compare type
  // compile expression, then compare type with param
...
void compileArguments(ObjectNode* paramList) {
  ...
  case SB_LPAR:
    ...
    // paramList empty but still receive left parenthesis
    if(paramList == NULL)
    ...
    while (lookAhead->tokenType == SB_COMMA) {
      ...
      //paramList ends but more arguments
      if(paramList == NULL)
        error(ERR_PARAMETERS_ARGUMENTS_INCONSISTENCY, currentToken->lineNo, currentToken->colNo);
      compileArgument(paramList->object);
    }
    
    // arguments end but more from paramList
...
void compileCondition(void) {
  // DONE: check the type consistency of LHS and RHS, check the basic type
...
Type* compileFactor(void) {
  ...
  case TK_IDENT:
    ...
    case OBJ_CONSTANT:
      ...
      break;
    case OBJ_VARIABLE:
      // ideas: check if variable is of array type
      // if true then compile indexes
      // else return type
